...
public class MyRenderKitUtils {
	...
	/**
	 * <p>
	 * <code>Boolean</code> attributes to be rendered using <code>XHMTL</code>
	 * semantics.
	 */
	 ...
	 * {@link #XHTML_ATTR_PREFIX} when rendering <code>XHTML Transitional</code>
	 * content.
	 */
	...
	/**
	 * <p>
	 * The maximum number of content type parts. For example: for the type:
	 * "text/html; level=1; q=0.5" The parts of this type would be: "text" -
	 * type "html; level=1" - subtype "0.5" - quality value "1" - level value
	 * </p>
	 */
	...
	/**
	 * The character that is used to delimit the type and subtype portions of a
	 * content type in an accept String. Example: text/html
	 * </p>
	 */
	 ...
	 * <code>attributesThatAreSet</code> List for optimized attribute rendering.
	 *
	 * IMPLEMENTATION NOTE: This must be kept in sync with the array in
	...
	/**
	 * IMPLEMENTATION NOTE: This must be kept in sync with the Key in
	 * UIComponentBase$AttributesMap and HtmlComponentGenerator.
	 *
	 * Hopefully JSF 2.0 will remove the need for this.
	 */
	...
	/**
	 * <p>
	 * Obtain and return the {@link ResponseStateManager} for the specified
	 * #renderKitId.
	 * </p>
	 *
	 * @param context
	 ...
	 *             if an exception occurs while trying to obtain the
	 *             <code>ResponseStateManager</code>
	 */
	public static ResponseStateManager getResponseStateManager(FacesContext context, String renderKitId)
		...
		if (renderKit == null) {
			...
			RenderKitFactory factory = (RenderKitFactory) RequestStateManager.get(context,
			...
			if (factory != null) {
				...
				if (factory == null) {
					throw new FacesException(
							"Unable to locate RenderKitFactory for " + FactoryFinder.RENDER_KIT_FACTORY);
				} else {
	...
	/**
	 * <p>
	 * Return a List of {@link javax.faces.model.SelectItem} instances
	 * representing the available options for this component, assembled from the
	 * set of {@link javax.faces.component.UISelectItem} and/or
	 * {@link javax.faces.component.UISelectItems} components that are direct
	 * children of this component. If there are no such children, an empty
	 * <code>Iterator</code> is returned.
	 * </p>
	 *
	 * @param context
	 *            The {@link javax.faces.context.FacesContext} for the current
	 *            request. If null, the UISelectItems behavior will not work.
	 * @param component
	 *            the component
	 ...
	 * @param writer
	 *            writer the {@link javax.faces.context.ResponseWriter} to be
	 *            used when writing the attributes
	 * @param component
	 *            the component
	 * @param attributes
	...
	public static void renderOnclick(FacesContext context, UIComponent component,
			Collection<ClientBehaviorContext.Parameter> params, String submitTarget, boolean needsSubmit)
					throws IOException {
		...
		renderHandler(context, component, params, handlerName, userHandler, behaviorEventName, submitTarget,
				needsSubmit, false);
	}
	...
	/**
	 * <p>
	 * Renders the attributes from {@link #BOOLEAN_ATTRIBUTES} using
	 * <code>XHMTL</code> semantics (i.e., disabled="disabled").
	 * </p>
	 *
	 * @param writer
	 *            writer the {@link ResponseWriter} to be used when writing the
	 *            attributes
	 * @param component
	 *            the component
	 * @throws IOException
	 ...
	 *            a <code>List</code> of attributes that are to be excluded from
	 *            rendering
	 * @throws IOException
	 *             if an error occurs writing the attributes
	 */
	public static void renderXHTMLStyleBooleanAttributes(ResponseWriter writer, UIComponent component,
			List excludedAttributes) throws IOException {
	...
	/**
	 * <p>
	 * Given an accept String from the client, and a <code>String</code> of
	 * server supported content types, determine the best qualified content type
	 * for the client. If no match is found, or either of the arguments are
	 * <code>null</code>, <code>null</code> is returned.
	 * </p>
	 *
	 * @param accept
	 *            The client accept String
	 * @param serverSupportedTypes
	 *            The types that the server supports
	 * @param preferredType
	 *            The preferred content type if another type is found with the
	 *            same highest quality factor.
	...
	/**
	 * @param component
	 *            the UIComponent in question
	 * @return <code>true</code> if the component is within the
	 *         <code>javax.faces.component</code> or
	 *         <code>javax.faces.component.html</code> packages, otherwise
	...
	/**
	 * <p>
	 * For each attribute in <code>setAttributes</code>, perform a binary search
	 * against the array of <code>knownAttributes</code> If a match is found and
	 * the value is not <code>null</code>, render the attribute.
	 * 
	 * @param context
	 *            the {@link FacesContext} of the current request
	 * @param writer
	 *            the current writer
	 * @param component
	 *            the component whose attributes we're rendering
	 * @param knownAttributes
	 *            an array of pass-through attributes supported by this
	 *            component
	 * @param setAttributes
	 *            a <code>List</code> of attributes that have been set on the
	 *            provided component
	 * @param behaviors
	 *            the non-null behaviors map for this request.
	 * @throws IOException
	 *             if an error occurs during the write
	 */
	private static void renderPassThruAttributesOptimized(FacesContext context, ResponseWriter writer,
			UIComponent component, Attribute[] knownAttributes, List<String> setAttributes,
		...
		if ((behaviorEventName != null) && !renderedBehavior) {
			...
			for (int i = 0; i < knownAttributes.length; i++) {
				...
				if ((events != null) && (events.length > 0) && (behaviorEventName.equals(events[0]))) {
					...
					renderHandler(context, component, null, attr.getName(), null, behaviorEventName, null, false,
							false);
				}
			}
	 ...
	 *             if an error occurs during the write
	 */
	private static void renderPassThruAttributesUnoptimized(FacesContext context, ResponseWriter writer,
			UIComponent component, Attribute[] knownAttributes, Map<String, List<ClientBehavior>> behaviors)
					throws IOException {
	...
	/**
	 * <p>
	 * Determines if an attribute should be rendered based on the specified
	 * #attributeVal.
	 * </p>
	 *
	 * @param attributeVal
	 *            the attribute value
	 * @return <code>true</code> if and only if #attributeVal is an instance of
	 *         a wrapper for a primitive type and its value is equal to the
	 *         default value for that type as given in the specification.
	 */
	...
	/**
	 * <p>
	 * This method expects a <code>List</code> of attribute names that are to be
	 * excluded from rendering. A <code>Renderer</code> may include an attribute
	 * name in this list for exclusion. For example, <code>h:link</code> may use
	 * the <code>disabled</code> attribute with a value of <code>true</code>.
	 * However we don't want <code>disabled</code> passed through and rendered
	 * on the <code>span</code> element as it is invalid HTML.
	 * </p>
	 *
	 * @param attributeName
	 *            the attribute name that is to be tested for exclusion
	 * @param excludedAttributes
	 *            the list of attribute names that are to be excluded from
	 *            rendering
	 * @return <code>true</code> if the attribute name is not in the exclude
	 *         list.
	 */
	...
	/**
	 * <p>
	 * This method builds a two element array structure as follows: Example:
	 * Given the following accept string: text/html; level=1, text/plain; q=0.5
	 * [0][0] 1 (quality is 1 if none specified) [0][1] "text" (type) [0][2]
	 * "html; level=1" (subtype) [0][3] 1 (level, if specified; null if not)
	 *
	 * [1][0] .5 [1][1] "text" [1][2] "plain" [1][3] (level, if specified; null
	 * if not)
	 *
	...
	private static String[][] buildTypeArrayFromString(String accept) {
		...
		for (int i = 0; i < types.length; i++) {
			...
			index += 1;
			// Check to see if our accept string contains the delimiter that is
			// used
			// to add uniqueness to a type/subtype, and/or delimits a qualifier
			// value:
			...
			if (token.contains(";")) {
				...
				for (int j = 1; j < typeParts.length; j++) {
					...
					// if "level" is present, make sure it gets included in the
					// "type/subtype"
				...
				quality = "not set"; // to identifiy that no quality was
										// supplied
			}
	 ...
	 * If a match is found, keep track of the highest quality factor. The end
	 * result is that for all matches, only the one with the highest quality
	 * will be returned.
	 * </p>
	 *
	 * @param clientContentTypes
	 *            An <code>array</code> of accept <code>String</code>
	 *            information for the client built from @{link
	 *            #buildTypeArrayFromString}.
	 * @param serverSupportedContentTypes
	 *            An <code>array</code> of accept <code>String</code>
	 *            information for the server supported types built from @{link
	 *            #buildTypeArrayFromString}.
	 * @param preferredContentType
	 *            An <code>array</code> of preferred content type information.
	 * @return An <code>array</code> containing the parts of the preferred
	 *         content type for the client. The information is stored as
	 *         outlined in @{link #buildTypeArrayFromString}.
	 */
	private static String[][] findMatch(String[][] clientContentTypes, String[][] serverSupportedContentTypes,
				...
				for (int cidx = 0, clen = clientContentTypes.length; cidx < clen; cidx++) {
					...
					if (browserType != null) {
						...
						if ((browserType.equalsIgnoreCase(serverType)) || (browserType.equals("*"))) {
							// types are equal or browser type is wildcard -
							// compare subtypes
							if ((clientContentTypes[cidx][2].equalsIgnoreCase(serverSupportedContentTypes[sidx][2]))
									|| (clientContentTypes[cidx][2].equals("*"))) {
								// subtypes are equal or browser subtype is
								// wildcard
								// found match: multiplicate qualities and add
								// to result array
								// if there was a level associated, this gets
		...
		// First, determine if we have a type that has the highest quality
		// factor that
	 ...
	 *            the original identifer that needs to be 'ECMA-ized'
	...
	public static void renderUnhandledMessages(FacesContext ctx) {
		...
		if (ctx.isProjectStage(ProjectStage.Development)) {
			...
			Renderer messagesRenderer = ctx.getRenderKit().getRenderer(HtmlMessages.COMPONENT_FAMILY,
	...
	public static String getImageSource(FacesContext context, UIComponent component, String attrName) {
		...
		if (resName != null) {
			...
			if (libName == null && resName.startsWith(
	...
	public static void appendProperty(StringBuilder builder, String name, Object value, boolean quoteValue) {
		...
		MyRenderKitUtils.appendQuotedValue(builder, name);
		builder.append(":");

		if (value == null) {
			builder.append("''");
		} else if (quoteValue) {
			MyRenderKitUtils.appendQuotedValue(builder, value.toString());
		} else {
	...
	private static boolean isBehaviorEventAttribute(Attribute attr, String behaviorEventName) {
		...
		return ((behaviorEventName != null) && (events != null) && (events.length > 0)
	...
	private static String getSubmitHandler(FacesContext context, UIComponent component,
			Collection<ClientBehaviorContext.Parameter> params, String submitTarget, boolean preventDefault) {
	...
	private static String getChainedHandler(FacesContext context, UIComponent component, List<ClientBehavior> behaviors,
			Collection<ClientBehaviorContext.Parameter> params, String behaviorEventName, String userHandler,
			String submitTarget, boolean needsSubmit) {
...
//		return script;
	...
	private static HandlerType getHandlerType(List<ClientBehavior> behaviors,
			Collection<ClientBehaviorContext.Parameter> params, String userHandler, boolean needsSubmit,
			boolean includeExec) {
